...
const [, , admin1Path, outputPath = "data/categories-map.generated.js", wardGeojsonPath = ""] = process.argv;

if (!admin1Path) {
  throw new Error("Usage: node scripts/build-categories-map-data.mjs <natural-earth-admin-1-geojson> [output] [mlit-n03-tokyo-geojson]");
}

const world = JSON.parse(readFileSync(admin1Path, "utf8"));
const wardGeojson = wardGeojsonPath ? JSON.parse(readFileSync(wardGeojsonPath, "utf8")) : null;
...
function tokyoSpecialWardFeature(feature) {
  const code = String(feature.properties?.N03_007 || "");
  return /^131(0[1-9]|1[0-9]|2[0-3])$/.test(code);
}

function tokyoSpecialWards(features = []) {
  const groups = new Map();
  features.filter(tokyoSpecialWardFeature).forEach((feature) => {
    const properties = feature.properties || {};
    const id = String(properties.N03_007 || "");
    const ward = groups.get(id) || { id, name: properties.N03_004 || id, paths: [] };
    const d = geometryToPath(feature.geometry, (point) => projectByBounds(kantoProjection, point), 0.045, 0.01, kantoProjection);
    if (d) ward.paths.push(d);
    groups.set(id, ward);
  });
  return [...groups.values()]
    .map((ward) => ({ id: ward.id, name: ward.name, d: ward.paths.join("") }))
    .filter((ward) => ward.d)
...
const categoriesMapData = {
  source: {
    ...
    note: wardGeojson
      ? "Made with Natural Earth and MLIT N03 administrative boundary data."
      : "Made with Natural Earth. Public domain vector data.",
    dataset: "ne_10m_admin_1_states_provinces",
    downloadedFrom: "https://github.com/nvkelso/natural-earth-vector",
    wardBoundaries: wardGeojson
      ? {
          name: "National Land Numerical Information Administrative Area Data",
          dataset: "N03-20250101_13",
          source: "Ministry of Land, Infrastructure, Transport and Tourism",
          terms: "https://nlftp.mlit.go.jp/ksj/gml/datalist/KsjTmplt-N03-2025.html"
        }
      : null
  },
  japan: {
  ...
  kanto: {
    ...
    wards: tokyoSpecialWards(wardGeojson?.features)
  }
};
...
writeFileSync(
  ...
  `// Generated by scripts/build-categories-map-data.mjs from Natural Earth public domain data` +
    `${wardGeojson ? " and MLIT N03 administrative boundary data" : ""}.\n` +
    `// Natural Earth terms: ${categoriesMapData.source.terms}\n` +
    `${wardGeojson ? `// MLIT N03 source: ${categoriesMapData.source.wardBoundaries.terms}\n` : ""}` +
